...
function M.new(options)
  ...
  function Html.header(s,level)
    ...
    -- this is to add local hyperlinks to some headers, this makes markdown TOC work
    local href = ''
    local t
    if type(s[1]) == 'table' then t = s[1] else t = s end
    if #t > 1 and #t < 5 then
       href = table.concat(t,'-'):lower()
       href = href:gsub("[ -]+","-")
       href = href:sub(1, -2)
    end    
    if href ~= '' then
       return {sep, "<h", level, "><a name=\"" ..  href  .."\">", s, "</a></h", level, ">"}
    end
